...
- Section 3.1, the unlearning objective: the training loss with its sign negated,
  applied to the target token sequences. Note that nothing else in the training
  procedure changes.
- Section 3.2: Extraction Likelihood and Memorization Accuracy, and the empirical
  definition of forgotten, fixed by comparison against validation data the model
  never saw. Note what that threshold certifies and what it leaves open.
- Section 4.3 and Table 4: forgetting difficulty and capability loss by data
  domain. Note which domains are reported as harder and the one-sentence
  hypothesis offered for why.
